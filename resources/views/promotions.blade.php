...
<head>
    ...
    <style>
        ...
        .container { margin-top: 20px; text-align:c center; }
        .container h1 { text-align: center; }
        .row { display: grid; grid-template-columns: repeat(4, 1fr); gap: 20px; }
        .col-md-4 { padding: 0.5rem;}
        .card { transition: transform 0.2s; border: 1px solid #dee2e6; border-radius: 0,25rem; }
        .card:hover { transform: scale(1.05); }
        .card-body { text-align: center;  }
        .card-title { font-size: 1.25rem; font-weight: bold; margin-bottom: 15px; }
        .card-text { margin-bottom: 10px; }
        .text-danger { font-size: 1.5rem; color: #000; padding: 5px;  margin-bottom: 10px; border-radius: 25%; background: yellow; font-weight: bold; }
        .btn-danger { background-color: red; border-color: #007bff; border-radius: 25px; color: white; padding: 3px; }
        .btn-primary:hover { background-color: #0056b3; border-color: #0056b3; }
        .card-img-top { max-width: 50%; height: 40; display: block; margin: 0 auto; }
...
<div class="encabezado">
    ...
    <div class="container">
        ...
        @if($promotions->isEmpty())
            <p>No hay promociones disponibles en este momesnto.</p>
        @else
            <div class="row">
                    ...
                    <div class="col-md-4">
                        <div class="card mb-4">
                            <div class="card-body">
                                ...
                                <img src="{{ asset('storage/' . $promotion->product->imagen) }}" class="card-img-top" style="width: 70%; display: block; margin: 0 auto;">
                                <p class="card-text"><strong>Marca:</strong> {{ $promotion->product->brand->name }}</p>
                                <p class="card-text">Descuento: {{ $promotion->discount_percentage }}%</p>
                                <span style="text-decoration: line-through;">${{ number_format($promotion->product->regular_price, 2) }}</span> 
                                <span class="text-danger">${{ number_format($promotion->getPriceAfterDiscount(), 2) }}</span>
                                <p class="card-text">Válido desde: {{ $promotion->start_date }} hasta {{ $promotion->end_date }}</p>
                                <a href="#" class="btn btn-danger"><svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-cart-dash-fill" viewBox="0 0 16 16">
                                ...
                                <label for="quantity-{{ $promotion->product->id }}"><strong>Cantidad:</strong></label>
                                <input type="number" id="quantity-{{ $promotion->product->id }}" class="form-control" name="quantity" value="1" min="1" style="width: 80px; margin-bottom: 10px;">
                            </div>
                        </div>
                    </div>
                @endforeach
